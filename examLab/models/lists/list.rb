...
class List
  ...
  def decrease(list, books, stationerys)
    list.all_products.uniq.each do |product|
      if product.class == Book
        id = books.id_by_name(product.name)
        books.decrease_quantity(list.count_product(product.name), id)
      else
        id = stationerys.id_by_name(product.name)
        stationerys.decrease_quantity(list.count_product(product.name), id)
      end
    end
  end
